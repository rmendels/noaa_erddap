...
def traverse_catalog_worker(args):
    ...
    url, max_depth, current_depth, thredds_version, verbose = args
...
def traverse_catalog_parallel(url, max_depth=5, thredds_version=None, verbose=False, max_workers=5):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        future = executor.submit(traverse_catalog_worker, 
                               (url, max_depth, 0, thredds_version, verbose))
        ...
        while futures:
            ...
            for future in done:
                try:
                    datasets, refs, current_depth = future.result()
